...
#define M1_IA_INFO (M1_I_A|ADC1_PREFIX|SIDE_A_PREFIX)
#define M1_IB_INFO (M1_I_B|ADC1_PREFIX|SIDE_B_PREFIX)
#define M1_IC_INFO (M1_I_C|ADC2_PREFIX|SIDE_A_PREFIX)
#define M1_UDC_INFO (M1_UDCB|ADC2_PREFIX|SIDE_B_PREFIX)
#define M1_AUX_INFO (M1_AUX|ADC2_PREFIX|SIDE_A_PREFIX)

/* Current sampling offset measurement filter window */
#define M1_ADC_OFFSET_WINDOW (3)

#define M1_ADC_AVG_NUM          2      /* 2^AVG_NUM conversions averaged */
#define M1_ADC_TRIGGER_DELAY 	1.0   /* [us] */
...
#define M1_FAULT_ALT1_NUM  (1)
#define M1_FAULT_ALT2_NUM  (2)
...
#define M1_LD		        (0.00028)  /* [H], d-axis inductance */
#define M1_LQ		        (0.00028)	 /* [H], q-axis inductance */
#define M1_R		        (0.135)	   /* [ohm], phase resistance */
#define M1_ENCODER_LINES	(131072/4)	 /* Equivalent Encoder lines per mechanical revolution, not used in TAMAGAWA Abs Hal driver */
#define M1_FLUX	             (0.017)/* [Wb] * motor flux */
#define M1_INERTIAL	         (0.000108)/* [kg*m^2] motor INERTIAL*/
#define M1_KT	             (0.12)/* [Nm/Arms] */
#endif
...
#define M1_N_MAX 	(3000.0F) /* [RPM], rotor mechanical speed scale */
...
#define M1_U_DCB_UNDERVOLTAGE (15.0F)  	/* [V], DC bus under voltage threshold */
#define M1_U_DCB_OVERVOLTAGE 	(45.0F)  	/* [V], DC bus over voltage threshold */
#define M1_N_OVERSPEED        (3500.0F) 	/* [RPM], mechanical over speed threshold */
...
#define M1_FOC_FREQ_VS_PWM_FREQ   (0.5)         /* FOC calculation is called every n-th PWM reload */
#define M1_PWM_DEADTIME           (0.8)       /* [us], Output PWM deadtime value in micro-seconds */
#define M1_FAST_LOOP_FREQ		  (float_t)(M1_PWM_FREQ*M1_FOC_FREQ_DIV_PWM_FREQ)  /* [Hz], fast loop control frequency */
#define M1_SLOW_LOOP_FREQ		  (8000.0F)   /* [Hz], slow loop control frequency */
#define M1_SLOW_LOOP_PER		  (float_t)(1000000/M1_SLOW_LOOP_FREQ)       /*us*/
#define M1_FOC_FREQ_DIV_PWM_FREQ   (2)         /* FOC calculation is called every n-th PWM reload */
...
#define M1_ALIGN_DURATION_TIME	(1.0F)  /* [s], alignment stage duration */
...
#ifdef M1_M_60ST400
#define M1_CLOOP_FREQ			(2000.0F)	/* [Hz], oscillating frequency */  // BANDWITH = 2000 hz
#endif
...
#else
//D-axis Controller - Parallel type
#define M1_D_KP_GAIN                       (2*PI*M1_CLOOP_FREQ*M1_LD )
#define M1_D_KI_GAIN                       (2*PI*M1_CLOOP_FREQ*M1_R/M1_FAST_LOOP_FREQ)
//Q-axis Controller - Parallel type
#define M1_Q_KP_GAIN                       (2*PI*M1_CLOOP_FREQ*M1_LQ )
#define M1_Q_KI_GAIN                       (2*PI*M1_CLOOP_FREQ*M1_R/M1_FAST_LOOP_FREQ)

#endif

#define M1_CURREQFW_FREQ                   (300.0F)          /* [Hz], cutoff frequency of IIR1 low pass filter for current request from speed pi */

#define M1_CURREQFW_IIR_B0				   WARP(M1_CURREQFW_FREQ, M1_SLOW_LOOP_FREQ)/(WARP(M1_CURREQFW_FREQ, M1_SLOW_LOOP_FREQ) + 2.0F)
#define M1_CURREQFW_IIR_B1				   WARP(M1_CURREQFW_FREQ, M1_SLOW_LOOP_FREQ)/(WARP(M1_CURREQFW_FREQ, M1_SLOW_LOOP_FREQ) + 2.0F)
#define M1_CURREQFW_IIR_A1				   (1.0F - M1_CURREQFW_IIR_B0 - M1_CURREQFW_IIR_B1)

//Speed Loop Control                    
//----------------------------------------------------------------------
#ifdef M1_M_60ST400
#define M1_SLOOP_FREQ			(40.0F)	/* [Hz], oscillating frequency */
#endif
//Speed Controller - Parallel type      
#if 1
#ifdef M1_M_2310P_LN_04K
  #define M1_SPEED_PI_PROP_GAIN              (0.01F)//(0.25F -> cut feq 250Hz   0.32 -> cut feq 500Hz)		/* proportional gain */0.31    BANDWITH = 270 hz
#endif
...
#ifdef M1_M_60ST400
  #define M1_SPEED_PI_PROP_GAIN              (0.2F)//(0.25F -> cut feq 250Hz   0.32 -> cut feq 500Hz)		/* proportional gain */0.31    BANDWITH = 270 hz
#endif

#define M1_SPEED_PI_INTEG_GAIN             (0.0001F)//(0.00065685F)	/* integral gain */    0.008
#else
#define M1_SPEED_PI_PROP_GAIN              (M1_INERTIAL*2*PI*M1_SLOOP_FREQ/M1_KT)
#define M1_SPEED_PI_INTEG_GAIN             (0.707*M1_INERTIAL*(2*PI*M1_SLOOP_FREQ*2*PI*M1_SLOOP_FREQ*2*PI*M1_SLOOP_FREQ)/(M1_KT*M1_CLOOP_FREQ*2*PI*M1_SLOW_LOOP_FREQ))
//#define M1_SPEED_DAMPING_FACTOR              10.0F     // from TI
//#define M1_SPEED_PI_INTEG_GAIN_ser           (M1_Q_KP_GAIN/(M1_SPEED_DAMPING_FACTOR*M1_SPEED_DAMPING_FACTOR*M1_LQ))
//#define M1_SPEED_PI_PROP_GAIN                (M1_SPEED_DAMPING_FACTOR*M1_SPEED_PI_INTEG_GAIN_ser*M1_INERTIAL/(1.5*M1_MOTOR_PP*M1_FLUX))
//#define M1_SPEED_PI_INTEG_GAIN               (M1_SPEED_PI_PROP_GAIN*M1_SPEED_PI_INTEG_GAIN_ser/M1_SLOW_LOOP_FREQ)
#endif
...
#define M1_CL_SPEED_RAMP			   	   (1000.0F) 		/* [RPM/s], (mechanical) speed accelerating rate during closed-loop */
#define M1_SPEED_CUTOFF_FREQ    		   (300.0F)          /* [Hz], cutoff frequency of IIR1 low pass filter for speed from sensor or observer */
#define M1_SPEED_LOOP_IQ_FWD_GAIN 		   (0.3F)		    /* [A], Iq feed forward gain */
#define M1_SPEED_DEEP_CUTOFF_FREQ    	   (120.0F)          /* [Hz], cutoff frequency of IIR1 low pass filter for speed from sensor or observer */

#define M1_CUR_FWD_CUTOFF_FREQ  		   (300.0F)   /* [Hz], cutoff frequency of IIR1 low pass filter */
...
#define M1_SPEED_DEEP_IIR_B0			   WARP(M1_SPEED_DEEP_CUTOFF_FREQ, M1_SLOW_LOOP_FREQ)/(WARP(M1_SPEED_DEEP_CUTOFF_FREQ, M1_SLOW_LOOP_FREQ) + 2.0F)
#define M1_SPEED_DEEP_IIR_B1			   WARP(M1_SPEED_DEEP_CUTOFF_FREQ, M1_SLOW_LOOP_FREQ)/(WARP(M1_SPEED_DEEP_CUTOFF_FREQ, M1_SLOW_LOOP_FREQ) + 2.0F)
#define M1_SPEED_DEEP_IIR_A1			   (1.0F - M1_SPEED_DEEP_IIR_B0 - M1_SPEED_DEEP_IIR_B1)
...
#define M1_QDC_POSITION_RAMP           		30.0 		 /* [Revolutions/s], position ramping rate for a position command */
#define M1_QDC_POSITION_CTRL_P_GAIN			10.0			 /* proportional gain for position controller */
#define M1_QDC_POSITION_CTRL_LIMIT	   		2000.0F		 /* [RPM], mechanical speed - position controller output upper limit. Lower limit is its negative value */
...
#define M1_SPD_FWD_CUTOFF_FREQ  		   (140.0F)   /* [Hz], cutoff frequency of IIR1 low pass filter */

#define M1_SPD_FWD_IIR_B0				   WARP(M1_SPD_FWD_CUTOFF_FREQ, M1_SLOW_LOOP_FREQ)/(WARP(M1_SPD_FWD_CUTOFF_FREQ, M1_SLOW_LOOP_FREQ) + 2.0F)
#define M1_SPD_FWD_IIR_B1				   WARP(M1_SPD_FWD_CUTOFF_FREQ, M1_SLOW_LOOP_FREQ)/(WARP(M1_SPD_FWD_CUTOFF_FREQ, M1_SLOW_LOOP_FREQ) + 2.0F)
#define M1_SPD_FWD_IIR_A1				   (1.0F - M1_SPD_FWD_IIR_B0 - M1_SPD_FWD_IIR_B1)

  
// Position & Speed Sensors Module - Optical Encoder
//-----------------------------------------------------------------------------------------------------------  

#define M1_QDC_TIMER_PRESCALER				10 //10			 /* Prescaler for the timer within QDC, the prescaling value is 2^Mx_QDC_TIMER_PRESCALER */
#define M1_QDC_CLOCK	 					132000000	 /* [Hz], QDC module clock, which is the bus clock of the system */
...
#define M1_QDC_LINE_RECIPROCAL_4_REV_FRAC_GEN         FRAC32(1.0/(4*M1_ENCODER_LINES)) /* The reciprocal of 4*encoder_lines, in Q1.31 format. 
                                                                           This is to expand the raw counter value into range 0~0x7fffffff, which is [0~1) in Q1.31 format  */
#define M1_QDC_LINE_RECIPROCAL_4_POS_GEN              ((0xffffffffU/(4.0*M1_ENCODER_LINES))*1024) /* This constant turns 4*encoder_lines into range 0~0xffffffff in Q22.10 format */
...
#define M1_OL_START_I 				(1.5F)   /* [A], current amplitude during startup */
...
//-----------------------------------------------------------------------------------------------------------  
#define M1_SCALAR_VHZ_FACTOR_GAIN  	(0.09F)  				 /* [v/Hz], voltage against electrical frequency */
...
//-----------------------------------------------------------------------------------------------------------  
#define M1_TAMAGAWA_POSITION_CNT_RESOLUTION (17)  /* Resolution of position counter (one turn), in unit of bits */
#define M1_TAMAGAWA_COM_BAUDRATE            (2500000) /* UART baud rate */
#define M1_TAMAGAWA_REC_DMA_CHANNEL         (0)
#define M1_TAMAGAWA_IF_USE_ABS_POSITION     1 /* TRUE:Use position counter value directly without initial position detection  */
#define M1_TAMAGAWA_ABS_POSITION_COMP       FRAC32(-36.614/180.0)
#define M1_TAMAGAWA_SPEED_CAL_FREQ           M1_SLOW_LOOP_FREQ*2
#define M1_TAMAGAWA_SPEED_CAL_CONST         ((M1_TAMAGAWA_SPEED_CAL_FREQ*30.0/M1_N_MAX)*8388608)  /* Q9.23 format */

#define M1_ABS_TO_ATT						(1.0F)		 /* attenuation for tracking observer, which is to estimate rotor speed from real QDC position */
#define M1_ABS_TO_FREQ						(300.0)		 /* [Hz], oscillating frequency for tracking observer */

#define M1_ABS_TO_KP_GAIN					(2.0F*M1_ABS_TO_ATT*2*PI*M1_ABS_TO_FREQ)
#define M1_ABS_TO_KI_GAIN					(2*PI*M1_ABS_TO_FREQ * 2*PI*M1_ABS_TO_FREQ/M1_PWM_FREQ)
#define M1_ABS_TO_THETA_GAIN				(1.0F/(PI*M1_PWM_FREQ))

// Sinc filter configuration
//-----------------------------------------------------------------------------------------------------------
#define M1_SINC_ORDER                      3
#define M1_SINC_DECIMATION_RATE            64           /* Decimation rate */
#define M1_SINC_IA_CHN                     0             /* Channel number for Ia */
#define M1_SINC_IB_CHN                     1             /* Channel number for Ib */
#define M1_SINC_IC_CHN                     2             /* Channel number for Ic */
#define M1_SINC_UDC_CHN                    3             /* Channel number for Udc */
#define M1_SINC_I_SCALE                    32.0          /* [A], the current that corresponds to the maximum measurable range of the external SigmaDelta ADC. E.g. the input range of ADC is 
                                                         -320mV ~ 320mV, and the shunt resistor is 10mOhm, the measurable current range is 32A */
#define M1_SINC_U_SCALE                    82.76         /* [v], the DC bus voltage that corresponds to the maximum measurable range of the external SigmaDelta ADC. */
#define M1_SINC_RSLT_RSHIFT_BITS           6             /* The maximum possible result is 24bit, this field defines how many lower bits are ignored */
#define M1_SINC_MODULE_CLOCK_PRESCALE      1             /* Sinc filter clock frequency is M1_SINC_MODULE_CLOCK_FREQ/(M1_SINC_MODULE_CLOCK_DIV+1)/(2^M1_SINC_MODULE_CLOCK_PRESCALE) */
#define M1_SINC_MODULE_CLOCK_DIV           4             /* Must be >= 2. Sinc filter clock frequency is M1_SINC_MODULE_CLOCK_FREQ/(M1_SINC_MODULE_CLOCK_DIV+1)/(2^M1_SINC_MODULE_CLOCK_PRESCALE) */


#define M1_SINC_MODULE_CLOCK_FREQ          132000000.0   /* [Hz], sinc filter module clock */
#define M1_SINC_OUTPUT_CLOCK_FREQ          (M1_SINC_MODULE_CLOCK_FREQ/(M1_SINC_MODULE_CLOCK_DIV*EXPONENT(2,M1_SINC_MODULE_CLOCK_PRESCALE)))
#define M1_SINC_DECIMATION_CLOCK_FREQ      (M1_SINC_OUTPUT_CLOCK_FREQ/M1_SINC_DECIMATION_RATE) /* [Hz] */
#define M1_SINC_FIFO_DEPTH                 (M1_SINC_DECIMATION_CLOCK_FREQ/M1_PWM_FREQ) /* Must be an integer! */
#define M1_SINC_RSLT_SCALE                 EXPONENT(M1_SINC_DECIMATION_RATE, M1_SINC_ORDER)/EXPONENT(2,M1_SINC_RSLT_RSHIFT_BITS) /* Maximum result value after the right shift */ 

// Delay after receving EtherCAT sync signal to trigger a slowloop ISR
#define M1_DELAY_AFTER_ETHERCAT_SYNC       2.5     /* [us] */
#endif

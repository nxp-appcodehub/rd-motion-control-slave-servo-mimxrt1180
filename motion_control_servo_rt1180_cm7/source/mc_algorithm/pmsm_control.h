...
typedef struct pi_controller_desat
{
	...
	float_t fltErrDeepFilter; /* Input */
...
typedef struct
{
	...
	float fltIGain;
	float fltInteg_1;
	float fltSat; // saturation portion
	float fltMax;
	float fltMin;
}PI_CTRL_FLT_T;
...
typedef struct trajectory_filter
{
	...
	int64_t     i64A;
	uint64_t    ui64B;   /* temporary variable for debug */
	uint64_t    ui64C;
	uint64_t    ui64D;
	uint64_t    ui64E;
	uint32_t    ui32Ah;   /* high 32bit of A */
	uint32_t    ui32Al;   /* low 32bit of A */
	int32_t     i32ASign; /* the sign of A */
	uint32_t    ui32Mh;   /* high 32bit of M */
	uint32_t    ui32Ml;   /* low 32bit of M */
	int32_t     i32MSign; /* the sign of M */
	...
	int32_t    i32Out_1;
	int32_t    i32M;    /* Internal memory */
	int32_t    i32M_1;  /* Memory of last step */
...
typedef struct mcs_pmsm_foc_a1
{
    ...
    GDFLIB_FILTER_IIR1_T_FLT sCurReqFWFilter;      /* Current request filter */
...
typedef struct mcs_trajectory_a1
{
	...
	int32_t i32Q16PosRamp_1;     /* Ramping output of position command */
	int32_t i32Q16PosErr;     /* Ramping output of position command */
...
typedef struct mcs_position_a1
{
	...
	float_t     fltSpeedRefLim;     /* Speed reference limit */
	...
	float_t		fltSpeedFwdNoGainFilter;    /* Feed-forward speed reference to speed loop without gain, electrical angular speed */
    ...
    int32_t     i32PosrefErr;
    int32_t     i32SpdForword;
    ...
    GDFLIB_FILTER_IIR1_T_FLT sSpdFwdFilter;

	int32_t     i32Q16PosCmd;      /* Desired position, used when trajectory is not needed in position loop */
	int32_t     i32Q16PosOffSet;      /* Desired position, used when trajectory is not needed in position loop */
...
extern float_t PI_CTRL_FLT(float_t fltIn,float_t fltFliterIn, PI_CTRL_FLT_T *ptr);
extern PI_CTRL_FLT_T sIqPiParamsTest;
extern PI_CTRL_FLT_T sIdPiParamsTest;
extern PI_CTRL_FLT_T sSpdPiParamsTest;
extern void PIController_Init(PI_CTRL_FLT_T *ptr);
/*!
